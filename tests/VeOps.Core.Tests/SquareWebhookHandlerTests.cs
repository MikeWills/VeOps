...
public class SquareWebhookHandlerTests
{
    ...
    private const string NotificationUrl = "https://veops.example/webhooks/square/1";
    ...
    public async Task ValidSignature_PaymentBelongsToDifferentTeam_ReturnsIgnored_AndDoesNotTouchPayment()
    {
        ...
        await using var dbContext = CreateContext();
        var teamA = await SeedTeamAsync(dbContext, notificationUrl: "https://veops.example/webhooks/square/1");
        var teamB = await SeedTeamAsync(dbContext, notificationUrl: "https://veops.example/webhooks/square/2");
