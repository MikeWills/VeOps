...
public class ParentCrumbRoleMirrorTests
{
    ...
    private static string? AuthorizeRolesOn(string pageName)
    {
        ...
        var type = typeof(RoleGroups).Assembly.GetTypes()
            .SingleOrDefault(t => t.Name == pageName + "Model" && t.Namespace?.StartsWith("VeOps.Web.Pages") == true)
    ...
    {
        var pagesRoot = Path.Combine(RepositoryRoot().FullName, "src", "VeOps.Web", "Pages");
