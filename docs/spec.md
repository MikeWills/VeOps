...
- **10-day expiration:** ⚠️ **Removed entirely 2026-08-25 — its own premise was wrong.** As written: `Payment.Status = Unpaid` and `Candidate.ApplicationDateEnteredUtc <= Today - 10 days` → set `Payment.ExpiredUnpaid = true`, send `PaymentExpirationNotice` email template (placeholders: `{{CandidateName}}`, `{{SessionDate}}`, `{{PaymentAmount}}`) **to Mike** (not the candidate), stop further reminders for that payment. But the team's own exam fee is required before testing, so it can never legitimately still be unpaid once an FCC application exists (or a retest result is marked) — Mike: "the only '10 day rule' is the lifetime of the application at the FCC ... No fee, no test." See `docs/payment-reminders.md` and CLAUDE.md's "No fee, no test" Known Constraint.
...
- **Session Manager:** full visibility/edit on their own team's sessions — candidate table (registration, payment, application, license status), manual actions:
  - resend reminder email, mark paid manually for edge cases, add/edit a candidate's FRN if missing at registration
  - "Mark session as completed" — one action, set once the testing is done for the day. Sets `Session.TestingCompletedUtc`/`TestingCompletedByUserId`, and bulk-sets `Candidate.Tested = true` for every candidate in the session still in a non-terminal `ApplicationStatus` (`Unmatched` or `Received`) — candidates already marked `Failed`/`NotTested` before this point are left alone. ~~For each candidate who passed this way and has `HasFelonyDisclosure = true`, automatically send the `FelonyDisclosureInstructions` email template.~~ **Removed 2026-08-11 (#221):** sending it here meant it could only ever arrive *after* the exam, when the candidate can no longer easily ask about it, and it went out as a side effect of a bulk status flip rather than because anyone decided to. It is a manual per-candidate action now, offered whenever a disclosure is declared — see `docs/email-reference.md`. The action still reports how many candidates are still owed the instructions, so removing the automation cannot silently leave someone with nothing
