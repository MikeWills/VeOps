...
using VeOps.Core.CandidateActions;
using VeOps.Core.Data;
using VeOps.Core.Discord;
using VeOps.Core.Email;
using VeOps.Core.Entities;
using VeOps.Core.Integrations;
using VeOps.Core.ExamResults;
using VeOps.Core.ExamTools;
using VeOps.Core.Ingestion;
using VeOps.Core.Jobs;
using VeOps.Core.Navigation;
using VeOps.Core.Messaging;
using VeOps.Core.Messaging.Scanners;
using VeOps.Core.Notifications;
using VeOps.Core.Payments;
using VeOps.Core.Reporting;
using VeOps.Core.Scheduling;
using VeOps.Core.Sessions;
using VeOps.Core.Square;
using VeOps.Core.Uls;
using VeOps.Core.VecSubmissions;
using VeOps.Core.VolunteerExaminers;
using VeOps.Core.Zoom;
using VeOps.Web;
...
// application name and key-ring path here MUST exactly match VeOps.Worker's own
...
// file itself.
//
// The application name below is deliberately still "VeSessionManager" and must stay that way. It is
// a key-derivation purpose string, not a display name: change it and every credential already
// encrypted under the live key ring becomes undecryptable. It would not throw or log — the
// legacy-plaintext fallback in EncryptedStringConverter returns the raw stored value when Unprotect
// fails, so every integration would quietly authenticate with a base64 blob. The 2026-09-06 rename
// to VeOps left it alone for exactly this reason; changing it needs a planned re-encryption pass,
...
builder.Services.AddScoped<IMessageTriggerScanner, CandidatePassedScanner>();
...
// its next tick. Registrations below mirror VeOps.Worker/Program.cs's own (same
...
// note in VeOps.Worker/Program.cs for the reasoning (#360). Short version: both hosts
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (!await scope.ServiceProvider.GetRequiredService<AppDbContext>().Users.AnyAsync(u => u.PasswordHash != null))
    {
        var refusal =
            ...
            $"  Create the first administrator with: dotnet VeOps.Web.dll {BootstrapAdminCommand.Switch} --email <email> --name <name>" + Environment.NewLine +
...
// VeOps.Web.Tests, which boots this app in-process and renders every page. Top-level
